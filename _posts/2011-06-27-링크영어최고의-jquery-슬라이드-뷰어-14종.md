---
title: '[링크] jQuery, javascript 슬라이드 플러그인/라이브러리 모음'
author: 안형우
layout: post
...
벌써 5년이나 된 글이다. 그 사이 IE6는 더이상 지원하지 않아도 되는 브라우저가 됐다. 이제 IE8도 점유율이 2% 정도로 떨어졌다. 그래서 더이상 브라우저 호환성에 대해서는 체크하지 않으려고 한다. IE8 정도나 체크해 보고 사용하시라.

또한, 워낙 오래 된 슬라이더들이 많고 새로 나온 슬라이더도 많아서 여기 있는 자료가 썩 유용한지 모르겠다. 그리고 최근에는 [bxSlider][8](MIT 라이센스)가 너무 좋아서 그것만 사용하고 있다.

슬라이더를 찾아 들어왔다면 그냥 bxSlider를 사용할 것을 추천한다. bxSlider가 아닌 다른 것을 찾아 들어온 것이라면 아래 슬라이더들을 참고해 보는 것도 나쁘진 않겠다.

## 2017-01-05

시간이 워낙 오래된 것들이라, 라이센스가 변한 경우가 있을 수 있다. 따라서 내가 적어 둔 라이센스만 보면 **안 된다.** 실사용을 할 것이라면 반드시 **라이센스를 확인하고 사용하라.**

-----

## 분류를 잘 보고 사용해야 한다

tutorial(연습용 예제)과 plugin은 차이가 있다는 걸 알고 사용하라. 

tutorial은 보고 따라하는 거다. 재사용성이 좋을 수도 있지만 안 좋을 확률이 높다. 연습용이기 때문이다. 따라서, tutorial 이라는 제목이 붙은 놈을 갖다 쓰는 건 까다로울 수 있다. 혼자 공부하는 용도로는 좋다.

플러그인은 개발자가 제품으로 내놓은 것이다. 여러 상황을 고려해서 잘 만들었을 가망이 높다. 하지만, 역시 개발자가 모든 상황을 다 대응해 뒀다고 생각지는 마시라.


## [플러그인] bxSlider, 반응형 만능 슬라이드 &#8211;  WTFPL 라이센스.

갤러리 슬라이드, 여러 아이템 슬라이드, 썸네일 슬라이드 등 거의 모든 기능이 있는 슬라이드다. 
...
터치로 작동하고, jQuery가 필요없다. 꽤 다양한 옵션을 제공한다.
...
## [튜토리얼] Slick Slideshow Using jQuery

얘는 내가 실제로 활용해 본 놈인데 썩 어렵지는 않았다. IE6에서도 깔끔하게 돌아간다.
...
## [플러그인] 터치에 최적화된 플러그인 jQuery TouchTouch &#8211; 무료(MIT), IE8 이상
...
## [플러그인] 첫화면에 제품 특징 소개 같은 거 할 때 적합해 보이는 슬라이드 플러그인. Sequence.js &#8211; 오픈소스, 개인 프로젝트는 무료(GPLv3 license)
...
## [플러그인] rcarousel, 반응형은 아니지만 만능 슬라이드 &#8211;  MIT 라이센스.

갤러리 슬라이드, 여러 아이템 슬라이드 등 거의 모든 기능이 다 있는 슬라이드다. 예전에 알았다면 환호했겠지만, bxSlide와 함께 발견한 탓에 환호를 하진 않았다. 
...
## [플러그인] Animated Responsive Image Grid &#8211; 갤러리는 아니고, 이미지를 격자로 늘어 놓고 애니메이션으로 전환.  CC BY 2.0 라이센스.(상업용 사용 가능)

라이센스는 [CC BY 2.0][15]([한글][16])이다. 사용할 때 저작자를 반드시 표시해야 한다. 슬라이드 갤러리는 아니지만 꽤 인상적으로 이미지들을 표시해 주기 때문에 갈무리한다. 동영상은 데모3과 데모5다.
