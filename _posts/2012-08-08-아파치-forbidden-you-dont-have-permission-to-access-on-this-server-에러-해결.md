...
## 자신의 프로젝트 디렉토리에 접근해도 된다고 적어 주자

이해했다면, 자신의 프로젝트 디렉토리를 허용해 주자. 나는 `apache2.conf`의 다른 디렉토리 설정들 밑에 아래처럼 추가했다.
...
`Require all granted`를 적었으면 되는데, `Options`와 `AllowOverride` 설정도 알아 두자.
...
아파치를 재시작해야 한다. 아래 세 명령어 중 하나로 하면 된다.
...
아파치에 디렉토리 접근을 허용한 뒤에도 접속이 안 되고, 에러 로그를 봤는데 퍼미션이 없다고 나오면 이제 아파치 사용자에게 해당 폴더의 퍼미션을 주면 된다.

아파치 프로세서를 실행하는 사용자는 일반 사용자가 아니다. 실행은 루트 권한으로 하지만, 루트 사용자 권한을 갖는 것도 아니다. 아파치를 돌리는 전용 사용자가 실행한다(우분투의 경우 `www-data`, ). 웹 서버가 일반 사용자의 파일을 함부로 읽거나 쓰면 안 되기 때문에, 딱 웹사이트 서비스에 필요한 디렉토리와 파일에만 접근하도록 사용자를 분리해서 권한을 주는 것이다. 그렇다면 웹서버용 폴더와 파일에는 당연히 아파치가 접근할 수 있어야 한다. 구체적으로는 폴더는 실행 권한, 파일은 읽기 권한, 업로드를 하거나 파일을 작성하려면 쓰기 권한까지 있어야 한다. 아래 세 가지 접근 전략 중 하나를 택할 수 있다.
