...
오역 대부분은 사소한 것이었지만 정 반대로 번역한 것도 한 문장 있었다. 조건부 선택자를 사용했을 때 브라우저 성능에 끼치는 영향을 다루는 부분인데, 올바른 번역은 "왜 `a` 태그에서 `.button`을 찾게 만드냐, 그냥 `.button`을 찾게 만들면 되지" 하는 것이다. 그런데 반대로 번역해 놨었다. 

치명적인 것은 아니지만, 케스케이드라는 단어가 사용된 부분이 있는데, 4년 전에는 케스케이드를 그냥 빼고 번역했다. 그래도 뜻이 통하기 때문이다. 하지만 이번에는 "케스케이드"라고 음차해서 넣고 설명을 붙였다. CSS에서 케스케이드가 하는 역할에 대해 분명히 인식하고 있는 것이 낫다는 생각을 하게 됐기 때문이다.
