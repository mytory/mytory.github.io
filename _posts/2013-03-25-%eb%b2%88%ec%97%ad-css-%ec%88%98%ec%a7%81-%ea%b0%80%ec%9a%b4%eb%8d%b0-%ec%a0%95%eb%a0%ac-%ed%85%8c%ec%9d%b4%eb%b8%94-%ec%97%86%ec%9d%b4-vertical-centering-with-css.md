...
tags:
  - CSS
---
뭔가 폼을 짤 일이 있었다. 보통은 테이블로 짰지만, 이번엔 테이블로 짜지 않고 label과 div를 이용해서 짜 봤다. 다 좋았는데, 수직으로 가운데 정렬을 하는 게 까다로웠다. 굳이 label을 가운데 정렬하지 않는 게 가장 좋은 것이라고 나는 생각한다. CSS에선 table 외에 수직 가운데 정렬(vertical-align)을 제대로 지원하지 않는다. 그렇다면 안 쓰면서 좋은 디자인을 하는 게 가장 좋을 일 아닌가?

그러나 디자인은 이미 나왔고 수직 가운데 정렬을 해야 하는 상황이었다. 에잇, 그냥 js로 margin을 계산하게 해 버렸다. (참고로 [수직 가운데 정렬을 해 주는 jQuery 플러그인][1]도 있다. 보통은 이걸 사용하는 게 속편한 것 같다.) 뭐 별 상관 없지. 하지만 궁금했다. CSS 기법은 없는지. 찾아 봤고, 찾았다. 원문은 [Vertical Centering with CSS][2]다. <a class="simple-footnote" title="정확히 말하면 저자가 약간 개선한 버전을 이메일로 보내 줬다. 그래서 좌측 링크와는 약간 다른 게 있을 거다. 저자가 보내 준 개선판의 제목은 CSS Vertical Centering (without tables!)이다." id="return-note-9727-1" href="#note-9727-1"><sup>1</sup></a> 원문은 2009년 2월 말에 씌어졌고, 이 때는 아직 IE8 베타 버전만 나와있던 때다. IE8 정식 버전은 2009년 3월 19일에 출시됐다. 그걸 참고하면서 읽기 바란다. 아래부터 번역.

&#8212;&#8212;&#8212;&#8212;&#8212;

# CSS 수직 가운데 정렬 (테이블 없이!)
...
일러 두기: 아마 &#8220;테이블을 사용하려던 건 아닌데&#8221; 하고 생각할지 모르겠다. 테이블을 사용할 때 주된 문제는 마크업을 시멘틱하게 유지해야 한다는 것이다. 테이블 코드를 짜지만 그게 사실 진짜 표는 아닌 경우, 테이블로 코딩하지 말아야 한다. (실제 표인 경우엔 테이블을 사용해도 아무 문제 없다.) `<div>`를 테이블로 생각하게 만들어서 생기는 유일한 문제는 브라우저가 일관되게 작동하지 못하게 된다는 점이다. 이건 미친 짓은 아니다. a에 `display:block;`을 주거나 `<h2>`에 `display:inline;`을 주는 것과 비슷한 것뿐이다.  <a class="simple-footnote" title="역자 주 &#8211; 스펙에 맞지 않게 요소를 사용하는 점이 걸린다는 뜻인 것 같다." id="return-note-9727-3" href="#note-9727-3"><sup>3</sup></a>
...
<pre>#wrapper {display:table;}
#cell {display:table-cell; vertical-align:middle;}</pre>
...
<pre>#content {position:absolute; top:50%; height:240px; margin-top:-120px; /* negative half of the height */}</pre>
...
<pre>#floater {float:left; height:50%; margin-bottom:-120px;}
#content  {clear:both; height:240px; position:relative;}</pre>
...
<pre>#content {position:absolute; top:0; bottom:0; left:0; right:0;
      margin:auto; height:240px; width:70%;}</pre>
...
<pre>#content {height:100px; line-height:100px;}</pre>
...
내가 좋아하는 방법은 3번째 방법 — float시킨 요소와 clear를 적용한 콘텐츠를 사용하는 방법이다. 이 방법엔 치명적인 결점이 하나도 없다. 콘텐츠가 `clear:both;`돼 있으니, 우리는 다른 요소를 그 위에 얹을 수도 있다. 그리고 윈도우가 접혀도 콘텐츠가 그 위를 덮지 않는다. [데모][5]를 봐라.
...
“hasLayout”이라는 주제 전체는 설명하기도, 이해하기도 아주 어렵다. 더 자세한 설명은 [Sitepoint’s Reference][8]를 다시 보면 된다.
