...
- 웹폰트 제공 업체인 폰트덱은 폰트 사용량 기반으로 과금하는데, 로컬 스토리지에 저장하면 사용량을 측정할 수 없다. 폰트덱은 친절하게도 폰트 로컬 저장을 허락해 줬고, 과금은 최근 페이지 노출 횟수를 기준으로 무난하게 하기로 해 줬다.
...
웹폰트를 위해서 나눔고딕의 경량화 버전을 만들었다. 경량화란 [자주 사용하는 한글 2,350자와 숫자, 영문, 특수문자][2350]를 뽑아서 웹폰트를 만드는 것을 말한다. 물론 이 2,350자 안에는 '똠'이나 '뷁', '햏' 같은 글자가 들어가 있지 않아서 해당 글자는 기본 폰트로 나오게 될 것이다.[^fn4]

[2350]: http://egloos.zum.com/banx/v/5144626
