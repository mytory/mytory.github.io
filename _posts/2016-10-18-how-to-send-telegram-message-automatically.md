...
봇으로 할 수 있는 건 무지 많다. 예시로 들고 있는 포브스 같은 곳의 봇은 끝내 준다. 인터랙티브한 상호작용이 있다. 

그러나 여기서 우리가 할 것은 그냥 새 글 알림만 보내 주는 것이니, 기능의 매우 일부만 사용하는 것이다. 그만큼 구현도 간편하고 제약도 적다. 
...
구현은 위 과정을 서버단 코드를 이용해 하는 것에 불과하다. PHP로 짠 코드는 이렇다.
    ...
    // 보낼 텍스트를 구성
    $telegram_text = urlencode("※ 새 글이 나왔습니다\n3년 10개월째 사용하는 아이폰 5에서 iOS 10을 돌리는 건 나쁘지 않은 듯\nhttp://mytory.net/2016/10/01/latest-iOS-is-good-on-iphone5.html");
    $api_code = '281680794:AAEQdVBa1aEEPIn9-DQ4LDlIVUSsmtUdiHc';
    
    // curl로 접속
    $curl_opt = array(
        CURLOPT_RETURNTRANSFER => 1,
        CURLOPT_URL => "https://api.telegram.org/bot{$api_code}/sendMessage?chat_id=@mytorychannel&text={$telegram_text}",
    );
    $curl = curl_init();
...
[^fn1]: 봇을 그룹에 집어넣을 수 있는 옵션(`/setjoingroups`)이 `Enabled`로 돼 있어야 하는데, 아마 기본값이 `Enabled`일 거다.
